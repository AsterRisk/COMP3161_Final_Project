...
<body>
    <?php
        ...
        include 'header.php';
        //
        $sql = "select members.member_id,groups.group_id, first_name, last_name, role, profile_pic_address from users join members join groups where users.user_id = members.member_id and members.group_id = groups.group_id and " . $_GET['group_id'] . "=members.group_id;";
        //echo($sql);
        $conn->prepare($sql);
        $results = $conn->query($sql);
        $sql = "select owner_id from groups where group_id = ". $_GET['group_id'] .";";
        $conn->prepare($sql);
        $owner_id = $conn->query($sql)->fetch_assoc()['owner_id'];
    ?>
    ...
    <?php
            ...
            foreach ($results as $row)
            {
               if ($row['member_id'] != 0)
               {
                   ...
                   {
                       $text = "<tr><td  class = 'w3-section'><img src = '". $row['profile_pic_address'] ."' style = 'height:50px;width:50px;border-radius:50%;margin-top:10px;'></td><td  class = 'w3-section'> ". $row['first_name'] ." </td>". " <td  class = 'w3-section'> ". $row['last_name'] ." </td> ". "<td  class = 'w3-section'> ". $row['role'] ." </td> ";
                   }
                   else
                   {
                       $text = "<tr><td  class = 'w3-section'><img src = '../assets/default_imgs/default_profile_picture.png' style = 'height:50px;width:50px;border-radius:50%;'></td><td  class = 'w3-section'> ". $row['first_name'] ." </td>". " <td  class = 'w3-section'> ". $row['last_name'] ." </td> ". "<td  class = 'w3-section'> ". $row['role'] ." </td> ";
                   }
                   ...
                   ?>
                    ...
                    <td>
                        <form method = "GET" action = "view_friend.php">
                            <?php
                                $hidden = "<input type = \"text\" style = \"display:none;\" name = 'friend_id' value = ".$row['member_id'].">";
                                echo $hidden;
                    ...
                    <td>
                        <form method = "POST" action = "remove_member.php">
                            ...
                            <?php
                            $hiddenid = "<input type = \"text\" style = \"display:none;\" name = 'member_id' value = ".$row['member_id'].">";
                            echo $hiddenid;
                            $hiddengroup = "<input type = \"text\" style = \"display:none;\" name = 'group_id' value = ".$row['group_id'].">";
                            echo $hiddengroup;
                            if($owner_id === $_SESSION['id']){
                                if(($row['member_id'] === $_SESSION['id']) and ($row['member_id']!=$owner_id)){
                                  $hidden = "<input type = \"text\" style = \"display:none;\" name = 'member_id' value = ".$row['member_id'].">";
                                  echo $hidden;
                                  $bttn = "<button class=\"w3-button w3-block w3-red w3-section fr_deny\" type = 'submit'>Leave <i class=\"fa fa-remove\" type = \"submit\"></i></button>";
                                  echo $bttn;
                                }
                    ...
                    <td>
                      <form method = "POST" action = "change_role.php">
                          ...
                          <?php
                          $hiddenrole = "<input type = \"text\" style = \"display:none;\" name = 'role' value = '".$row['role']."'>";
                          echo $hiddenrole;
                          $hiddenid = "<input type = \"text\" style = \"display:none;\" name = 'member_id' value = ".$row['member_id'].">";
                          echo $hiddenid;
                          $hiddengroup = "<input type = \"text\" style = \"display:none;\" name = 'group_id' value = ".$row['group_id'].">";
                              ...
                              if(($row['member_id'] === $_SESSION['id']) and ($row['member_id']!=$owner_id)){
                                ...
                                echo $bttn;
                                  ...
                                  if($row['member_id'] != $_SESSION['id']){
                                    $hidden = "<input type = \"text\" style = \"display:none;\" name = 'member_id' value = ".$row['member_id'].">";
                                    echo $hidden;
                                    if($row['role'] === 'Content Editor'){
                                      $bttn = "<button class=\"w3-button w3-block w3-red w3-section fr_deny\" type = 'submit'>Demote</button>";
                                    }
                                    else {
                                      $bttn = "<button class=\"w3-button w3-block w3-green w3-section fr_accept\" type = 'submit'>Promote</button>";
                                    }
                                    echo $bttn;
                                  }
