...
<body>
    <?php
        ...
        include 'header.php';
        //
        $sql = "select user_id, first_name, last_name, groups.group_id, group_name, group_dp_location, role from groups inner join (users inner join members on users.user_id = members.member_id) on members.group_id = groups.group_id where user_id = ". $_SESSION['id'] . ";";
        //echo($sql);
        $conn->prepare($sql);
    ...
    <?php
            ...
            foreach ($results as $row)
            {
                   ...
                   {
                       $text = "<tr><td><img src = '". $row['group_dp_location'] ."' style = 'height:50px;width:50px;border-radius:50%;'></td><td> ". $row['group_name'] ." </td>". " <td> ". $row['role'] ." </td> ";
                   }
                   else
                   {
                       $text = "<tr><td><img src = '../assets/default_imgs/default_group.png' style = 'height:50px;width:50px;border-radius:50%;'></td><td> ". $row['group_name'] ." </td>". "<td> ". $row['role'] ." </td> ";
                   }
